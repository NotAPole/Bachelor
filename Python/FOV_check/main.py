...
class Camera:
    def __init__(self, x, y, z, FOV, focus):
        self.x = x
        self.y = y
        self.z = z
        self.FOV = FOV
        ...
        for j in range(step):
            for k, l in enumerate(equations):
                ...
                if crossingPlane:
                    ...
                    conObs = (corners_x[0] - 0.1) <= x[j] <= (corners_x[1] + thresholdPoint) and \
...
def show_plots(seenPoints, obstructedPoints, palletFaces, cameras):
...
def main():
    ...
    #cameras.append(Camera(40, 60, -50, (10, 10), (0, 0, 0)))
    cameras.append(Camera(-20, -20, 7, (10, 10), (0, 0, 0)))
    ...
    for i in cameras:
        ...
        for j in remainingPoints:
            ...
            if pointObstructed:
                ...
                seenPoints.append(j)
            cameraProgress += 1
            print("Camera " + str(currentCamera) + " " + str(round(cameraProgress / len(remainingPoints) * 100, 2)) + "% done")
        remainingPoints = obstructedPoints
        currentCamera += 1

    print("Total time used:", time.time() - startTime)
    show_plots(seenPoints, obstructedPoints, p1.faces, cameras)
